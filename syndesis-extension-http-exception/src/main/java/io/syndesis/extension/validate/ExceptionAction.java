...
@Action(id = "Exception handling", name = "Exception", description = "Add a http exception catch step", tags = {
    "exception",
    "extension"
})
@ConfigurationProperties({
    ...
    @ConfigurationProperty(
        name = "delay",
        displayName = "Acceptable delay waiting for the response",
        description = "MS to wait before defaulting to standard behaviour",
        type = "String",
        required = false)
})
public class ExceptionAction implements Step {
    ...
    @Override
    public Optional < ProcessorDefinition < ? >> configure(CamelContext context, ProcessorDefinition < ? > route, Map <
        ...
        return Optional.of(route.process((Exchange exchange) -> {
            try {
                ...
                con.setConnectTimeout(delay); //set timeout to 5 seconds
